...
DETAIL_FEATURES = [
    ...
    ("TODO-003", ["#TODO-LIST", "#LINK-DETAIL"], "할일", "할 일 완료 체크", "Must",
     "POST /todos/:id/complete", "MVP", "기획완료", "낙관적 UI",
     [
        ("설명", "할 일 카드 좌측 체크박스 또는 상세에서 토글. 낙관적 UI로 즉시 ✓ 표시 후 서버 UPDATE"),
        ("입력", "todoId, completed: boolean"),
        ("출력", "link_todos.completed_at UPDATE + Realtime"),
        ("정책", "[낙관적 UI] 즉시 ✓ 후 UPDATE 호출, 실패 시 롤백\n[반복 알림] MVP는 \"전체 종료\"만(EventBridge Rule 일괄 제거). 회차별 완료는 v1.x"),
