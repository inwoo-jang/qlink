...
DETAIL_FEATURES = [
    ...
    # ----- LINK-007 -----
    ("LINK-007", ["#LINK-DETAIL", "#SETTINGS-NOTIFICATION"], "링크", "알림 예약", "Should",
     "예약 알림 API\nFCM Topic 발송", "일부포함", "기획완료",
     "결정: FCM Topic으로 모든 디바이스 발송 + 모바일/PC 채널 분리 옵션. 중복은 사용자 토글로 회피",
     [
        ("설명", "링크별 reminder_at 설정. EventBridge가 시간에 Lambda 트리거 → FCM Topic으로 사용자의 모든 활성 디바이스에 푸시 발송"),
        ("입력", "reminder_at (datetime, 미래)"),
        ("출력", "links UPDATE + 예약 등록 + 시간에 푸시"),
        ("정책", "[권한] 푸시 권한 미허용 시 인앱 토스트로 폴백\n[분배] FCM Topic으로 모든 활성 디바이스에 푸시\n[채널 분리] 설정에 \"모바일 알림 받기\" / \"데스크톱 알림 받기\" 토글 — 사용자가 채널별 ON/OFF"),
        ("예외", "- 과거 시각: 픽커 차단\n- 링크 삭제됨: 발송 안 함\n- 푸시 발송 실패: 24시간 후 1회 재시도\n- 같은 알림 중복 노출: 사용자가 한쪽 채널 OFF로 회피"),
    ]),
    # ----- QR-001 -----
