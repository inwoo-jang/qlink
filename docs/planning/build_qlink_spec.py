...
LIST_ROWS = [
    ...
    ("AUTH-003", "인증", "게스트 모드", "가입 없이 샘플 데이터로 즉시 체험", "MVP", "Must", "US-011"),
    ...
    ("THEME-001", "설정", "다크/라이트 + 강조색", "시스템·다크·라이트 + 4색", "일부포함", "Should", "US-012"),
...
DETAIL_FEATURES = [
    ...
    ("AUTH-001", ["#AUTH-LOGIN", "#AUTH-SIGNUP", "#AUTH-EMAIL-VERIFY"], "인증", "이메일 회원가입", "Must",
     "회원가입 API\n이메일 인증 코드 발송 API\n인증 코드 검증 API\n프로필 생성 트리거", "MVP", "기획완료",
     "TBD: 인증 방식 — 6자리 코드 vs 메일 링크 클릭. 인우님 결정 필요",
     [
    ...
    # ----- AUTH-003 -----
    ("AUTH-003", ["#AUTH-LOGIN", "#HOME"], "인증", "게스트 모드", "Must",
     "(클라이언트 처리)", "MVP", "기획완료",
     "TBD: 가입 시 게스트 데이터 마이그레이션 — 자동 vs 모달 선택. 일단 모달로",
     [
        ("설명", "가입 없이 샘플 데이터로 모든 기능 체험. 폴더 4개 + 공유 폴더 2개 + 링크 ~14개 시드. 로컬스토리지에만 저장. 가입 시 데이터 마이그레이션 옵션 제공"),
        ("입력", "\"가입 없이 둘러보기\" 버튼 클릭"),
        ("출력", "- 시드 데이터가 #HOME에 표시\n- 헤더에 \"게스트 모드\" 배지 + \"가입하기\" 버튼"),
        ("정책", "[로컬 저장 대상] 모든 데이터 (POLICY-LOCAL 참고)\n[동기화 비활성] SYNC-001 비활성 (가입 후 마이그레이션)\n[기능 차단] 공유 폴더 만들기/초대 수락은 가입 유도 모달"),
        ("예외", "- 게스트에서 공유 폴더 시도: '가입하면 친구와 함께 큐레이션할 수 있어요'\n- 게스트에서 동기화 시도: '가입하면 모든 디바이스에서 이어볼 수 있어요'\n- 로컬스토리지 한도 초과(5MB): '둘러보기 데이터가 가득 찼어요. 가입하면 안 차요'"),
    ]),
    # ----- AUTH-004 -----
    ...
    ("LINK-004", ["#LINK-ADD", "#LINK-DETAIL"], "링크", "AI 한 줄 요약 + 자동 태그", "Must",
     ...
     "TBD: 외부 세션 우선 vs 사용자 API 키 우선 — 인우님 결정 필요",
     [
    ...
    # ----- LINK-007 -----
    ("LINK-007", ["#LINK-DETAIL"], "링크", "알림 예약", "Should",
     "예약 알림 API\nFCM/APNs 발송", "일부포함", "기획완료",
     "TBD: 멀티 디바이스에서 같은 알림이 중복 노출될 위험 — FCM topic 또는 device_id 단위 분배",
     [
        ("설명", "링크별 reminder_at 설정. EventBridge가 시간에 Lambda 트리거 → FCM/APNs 푸시 발송"),
        ("입력", "reminder_at (datetime, 미래)"),
        ("출력", "links UPDATE + 예약 등록 + 시간에 푸시"),
        ("정책", "[권한] 푸시 권한 미허용 시 인앱 토스트로 폴백\n[동기화] 알림 시각은 모든 디바이스에 발송"),
    ...
    ("SYNC-001", ["(전 화면 백그라운드)"], "플랫폼", "모바일↔웹 자동 동기화", "Must",
     ...
     "TBD: WebSocket vs SSE vs polling. AWS API Gateway WebSocket 또는 AppSync Subscriptions 검토. CRDT 도입 여부",
     [
    ...
    ("IMPORT-001", ["#SETTINGS-IMPORT", "#IMPORT-UPLOAD", "#IMPORT-PREVIEW", "#IMPORT-PROGRESS"], "플랫폼", "크롬 즐겨찾기 가져오기 (웹 전용)", "Should",
     ...
     "TBD: AI 일괄 요약 비용 — 외부 세션이면 사용자 부담 0, API면 큰 비용. 사용자 동의 모달 필요",
     [
    ...
    ("SHARE-002", ["#SHARE-ACCEPT"], "공유 폴더", "공유 폴더 초대 수락", "Should",
     ...
     [
        ...
        ("예외", "- 토큰 만료: '유효하지 않거나 만료된 초대 링크입니다'\n- 이미 멤버: 토스트 + 폴더 이동\n- 본인이 오너: '본인의 폴더예요'\n- 멤버 한도(TBD): '참여할 수 없어요. 오너에게 문의하세요'"),
    ]),
    # ----- SHARE-003 -----
    ...
    ("SETTINGS-003", ["#SETTINGS-PASSWORD"], "설정", "비밀번호 변경", "Must",
     "비밀번호 변경 API", "MVP", "기획완료",
     "TBD: 변경 후 다른 디바이스 강제 로그아웃 여부",
     [
        ("설명", "현재 비밀번호 검증 → 새 비밀번호 변경. 변경 후 다른 디바이스 로그아웃 (선택, TBD)"),
        ("입력", "current_password, new_password (8~64자, 영문+숫자)"),
        ("출력", "비밀번호 변경 + 토스트 \"변경됐어요\""),
        ("정책", "[보안] 현재 비밀번호 검증 필수\n[선택] 변경 후 다른 디바이스 강제 로그아웃 옵션 (TBD)"),
        ("예외", "- 현재 PW 불일치: '현재 비밀번호가 일치하지 않아요'\n- 새 PW 정책 미충족: 안내\n- 5회 실패: 잠시 차단"),
    ]),
    # ----- SETTINGS-004 -----
    ("SETTINGS-004", ["#SETTINGS-RESET"], "설정", "데이터 초기화", "Must",
     "데이터 일괄 삭제 API", "MVP", "기획완료",
     "TBD: 30일 soft delete vs 즉시 hard delete (개인정보보호법 검토)",
     [
        ...
        ("예외", "- 비밀번호 불일치: 모달 안에서 '비밀번호가 일치하지 않아요'\n- 처리 중 네트워크 오류: 부분 삭제 위험 → 서버는 트랜잭션 보장"),
    ]),
    # ----- THEME-001 -----
    ("THEME-001", ["#SETTINGS-THEME"], "설정", "다크/라이트 + 강조색", "Should",
     "프로필 업데이트 API", "일부포함", "기획중", "",
     [("설명", "시스템·다크·라이트 + 강조색 4종(핑크·블루·퍼플·그린). 사용자별 저장. 로그인 화면은 항상 라이트+핑크(브랜드)")]),
    # ----- AI-001 -----
...
def build_workbook(out_path: str):
    ...
    ws_h.cell(row=2, column=3, value="최초 작성. 40개 기능(Must 19 / Should 8 / Could 5 / Won't 3 / Policy 4 / 보류 1). 모바일·웹 동시 PWA + 모바일↔웹 동기화 + 크롬 즐겨찾기 import 포함")
    ws_h.cell(row=2, column=4, value="인우")
    for c in range(1, 5):
        ws_h.cell(row=2, column=c).alignment = WRAP_TOP
